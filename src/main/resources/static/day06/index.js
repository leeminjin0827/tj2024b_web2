...
// JS의 기본 문법 정리 , ( reaact 에서 필요한 기본 문법 )
...
function fun1( param1 , param2 ){ }
    // 정의2 : 익명 함수 , 변수/상수 저장해서 사용한다. , 이벤트 함수 사용
...
console.log( obj3.age3 );
    // 4. 리스트 : [ value , value , value , value , value ]
...
// [4] 백틱 : 문자열 템플릿 , +연산자 가 아닌 `(백틱) 안에 ${}를 이용하여 문자열과 표현식을 연결할 때 사용.
    // 백틱 `${ 선언부 불가능 }` , 
        ...
        // 표현부 : 1 + 3 / sum() / 삼항연산자 / 변수호출/함수호출/객체호출 등등 가능
const str4 = `react`;
...
// [7] 삼항연산자 , 조건 ? 참 : 거짓
    // 1. 삼향연산자
const loginState1 = true;
console.log( loginState1 == true ? '로그인상태' : '비로그인상태' )

// [8] 단축 평가(연산)
const loginState2 = false;
        // 조건(A) && 참(B) : 만약에 조건이 true 이면 B 반환 false 이면 A 반환
        // 조건(A) || 거짓(B) : 만약에 조건이 true 이면 A반환 false 이면 B 반환
console.log( loginState1 && '로그인상태' ) // A가 true 이라서 B가 반환 // 로그인상태
console.log( loginState2 && '로그인상태' ) // A가 false 이라서 A가 반환 // false
console.log( loginState1 || '비로그인상태' ) // A가 true 이라서 A가 반환 // true
console.log( loginState2 || '비로그인상태' ) // A가 false 이라서 B가 반환 // 비로그인상태

// [9] 비동기 = 통신요청 보내고 응답이 오기전에 다른 코드 처리 , 동기 = 통신요청 보내고 응답이 올때까지 대기상태
    // 비동기
const fun8 = ( ) => {
...
console.log( arr10 );
    // 5. filter : 조건에 맞는것만 return 할 수 있다.
const arr11 = arr9.filter( ( value , index ) => { return value % 4 == 0 } ) // 짝수
console.log( arr11 );