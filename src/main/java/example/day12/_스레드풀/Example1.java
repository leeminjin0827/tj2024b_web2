...
public class Example1 {
    ...
    public static void main(String[] args) {
        ...
        for( int i = 1; i <= 10; i++){
            // (1) 작업 스레드 생성 , Thread thread = new Thread( 인터페이스명(){ 추상메소드구현 });
            String name = "thread " + i;
            Thread thread = new Thread( new Runnable(){
                @Override
                public void run() {
                    try { Thread.sleep(1500); } // 1.5초 일시정지 , Thread.sleep(밀리초)
                    catch ( Exception e ) { System.out.println( e ); }
                    System.out.println("[1]작업 스레드 : 실행중 " + name );
                }
            } ); // 작업 스레드 정의 end
            // (2) 작업 스레드 실행
        ...
        for( int i = 1; i <= 10; i++ ){
            ...
            executorService.submit( new Runnable() {
                @Override
                public void run() {
                    try { Thread.sleep(2500); } // 1.5초 일시정지 , Thread.sleep(밀리초)
                    catch ( Exception e ) { System.out.println( e ); }
                    System.out.println("[2]작업 스레드 : 실행중 " + name );
                }
            }); // 작업 스레드 정의 end
        } // for end
        // (4) 스레드풀 종료 , .shutdown();
