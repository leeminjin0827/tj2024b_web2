...
public class RestController2 {
    ...
    // [1] @ResponseBody
    // - 메소드의 반환(return)값을 HTTP 응답 본문(BODY) 으로 주로 application/json 타입으로 응답 자동으로 변환
    ...
    @GetMapping( "/doget3" )
    // @ResponseBody // 자동으로 함수 return 값을 HTTP Response 를 application/json 로 응답
    public int doGet3(){ return 100; } // f end (200) application/json : 100

    @GetMapping("/doget4")
    // @ResponseBody // 현재 클래스가 @RestController 이면 생략 가능 하다.
    public String doGet4(){ return "안녕 나는 스프링이야."; } // f end // (200) text/plain : 안녕 나는 스프링이야.
    ...
    public Map< String , String > doGet5(){
        ...
        map.put("name" , "강호동");
        return map;
    } // f end // (200) application/json : { name : 강호동 }
