...
public class RestController3 {
    ...
    public void method1( HttpServletRequest req , HttpServletResponse resp ) throws Exception {
        System.out.println("RestController3.method1");  // soutm 자동완성 : 현재 함수명 출력 함수
        // (1) 서블릿 방식의 쿼리스트링 받기.
    ...
    // -> queryString 매개변수명 과 함수 매개변수명과 동일하면 @RequestParam 생략 하면 @ModelAttribute 자동으로 적용
    // 2. ( name = "name" , defaultValue = "홍길동" ) 타입 매개변수명
    // -> 만약에 queryString 매핑할 매개변수명이 존재하지 않으면 기본값 설정
    // 3. ( required = true )
    // -> 기본값이 true 하며 , 쿼리스트링 매개변수 필수 여부 설정 , 만약에 쿼리스트링 매개변수가 없으면 400(bad request 잘못된요청) 오류

    @GetMapping("/doget2") // http://localhost:8080/day02/doget2?name=유재석
    public boolean method2( @RequestParam( name = "name" , defaultValue = "홍길동" , required = false ) String name ) { // 매핑함수의 매개변수를 자동으로 쿼리스트링의 매개변수를 매핑/연결 한다.
    ...
    @GetMapping("/doget3") // http://localhost:8080/day02/doget3?name=유재석&age=15
    ...
    @GetMapping("/doget4") // http://localhost:8080/day02/doget4?name=유재석&age=15
    public String method4(@RequestParam Map<String,String> map ){ // 여러개의 매개변수를 하나의 map 으로 받을 수 있다.
    ...
    // 생략가능(기본값)                    명시해야한다.
    // 개발자가 만든타입(DTO)               공식적타입 만 지원 : 기본타입+List.Map.String
    // 쿼리스트링 , form첨부파일             쿼리스트링 지원

    // + DTO 사용시 쿼리스트링 매개변수 와 멤버변수 명이 동일한다.
    @GetMapping("/doget5")
    public boolean method5( @ModelAttribute MemberDto memberDto ){ // 여러개의 매개변수를 하나의 DTO로 받을 수 있다.
