...
import { login } from "../rejuxs/userSlice";
export default function Login( props ){
    ...
    const onInputChange = (e) => { setMemberInfo({ ...memberInfo , [e.target.name] : e.target.value }); }
    ...
    const onLogin = async () => {
        ...
        if( result == true ){ 
            // (4) 로그인 성공할 경우 로그인 성공한 회원정보를 가져오기
            const response2 = await axios.get('http://localhost:8080/api/member/info' , { withCredentials : true } )
            alert('로그인성공');
            navigate("/");
            // (********************) 리덕스 ( 전역변수 ) 사용하기 (********************)
            // (2) useDispatch 함수를 이요한 리듀서 함수 액션하기. // 로그인 액션(함수)에 (로그인한) 회원정보를 대입한다. // 전역변수 대입
