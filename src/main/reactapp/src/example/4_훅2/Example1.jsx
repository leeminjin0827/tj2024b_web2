...
export default function Example1( props ) {
    ...
    // (1) 의존성배열이 없을때 useEffect , 실행시점 : 1.탄생(최초실행) 2.인생(재렌더링)
        // -> 차이점 : 모든 state 변경될때 재실행 된다.
    ...
    // (2) 의존성배열이 빈배열 일때 useEffect , 실행시점 : 1.탄생(최초실행) 2.X
        // -> 차이점 : 재렌더링이 아닌 컴포넌트 최초 1번만 실행해야 하는 경우에 사용된다.
    useEffect( () => {
        console.log( '이펙트 함수 실행2' )
    }, [] ) // []의존성배열이 빈배열 일때.

    // (3) 의존성배열에 state변수가 존재할때 useEffect , 실행시점 : 1.탄생(최초실행) 2.인생(재렌더링)
        // -> 차이점 : 의존성배열에 존재하는 state 변수가 변경될때만 재실행이 된다.
        //            의존성배열에 존재하지 않는 state변수가 변결될때는 재싱행이 안된다.
